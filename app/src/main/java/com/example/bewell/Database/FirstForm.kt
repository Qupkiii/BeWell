...
class FirstForm constructor() : DataHandler() {
    ...
    override fun uploadToFirebase() {
        ...
        db.collection(outerCollection).document(uid.toString()).collection("FirstForm").document(id)
            .set(data).addOnFailureListener { e ->
                throw Exception("Upload failed (FirstForm): ${e.message}")
            }
    }
}